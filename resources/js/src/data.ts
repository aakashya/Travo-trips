...
// Named waypoint images used by RouteJourney's Manali/Valley of Flowers/Udaipur route maps.
...
function convertAndamanToTripDetails(pkg: AndamanPackage): TripDetails {
  ...
  return {
    ...
    price: `₹${applyMarkup(samplePricing.perPersonPrice, pkg.id).toLocaleString('en-IN')}`,
    duration: pkg.duration,
    upcomingDeparture: "Daily Departure Available",
...
function convertGoaToTripDetails(pkg: GoaPackage): TripDetails {
  return {
    ...
    name: pkg.title,
    subtitle: pkg.subtitle,
    price: `₹${applyMarkup(4200, pkg.id).toLocaleString("en-IN")}`,
    duration: pkg.duration,
...
function convertNepalToTripDetails(pkg: NepalPackage): TripDetails {
  ...
  return {
    ...
    name: pkg.title,
    subtitle: pkg.subtitle,
    price: `₹${applyMarkup(lowestPrice, pkg.id).toLocaleString("en-IN")}`,
    duration: pkg.duration,
...
function convertBhutanToTripDetails(pkg: BhutanPackage): TripDetails {
  return {
    ...
    name: pkg.title,
    subtitle: pkg.subtitle,
    price: `₹${applyMarkup(pkg.pricePerPerson, pkg.id).toLocaleString("en-IN")}`,
    duration: pkg.duration,
...
// Manali, Valley of Flowers, and Udaipur are the original three trips and were inserted
// first into TRIPS_DATA, but the catalogue and homepage listings should show them last.
const TRIPS_SHOWN_LAST = new Set(["manali", "valley-of-flowers", "udaipur-lakes"]);
export const TRIPS_LIST: TripDetails[] = [
  ...Object.values(TRIPS_DATA).filter((trip) => !TRIPS_SHOWN_LAST.has(trip.id)),
  ...Object.values(TRIPS_DATA).filter((trip) => TRIPS_SHOWN_LAST.has(trip.id)),
];
