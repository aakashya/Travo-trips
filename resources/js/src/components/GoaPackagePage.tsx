...
export default function GoaPackagePage({ packageId, onNavigate, onOpenBooking }: GoaPackagePageProps) {
  ...
  const baseRatePerPerson = useMemo(() => {
    ...
    return applyMarkup(matched.perPersonPrice, pkg.id);
  }, [activeCategory, selectedPaxCount, pkg.id]);
...
Please check availability and send booking confirmation details.`;
  ...
  const handleBookNow = () => {
    onOpenBooking(pkg.id, { paxCount: selectedPaxCount, starRating: activeCategory.starRating });
  };
                ...
                >
                  {activeCategory.pricingByPax.map((p) => (
                    <option key={p.paxMin} value={p.paxMin}>
                      {p.sharingType} ({p.paxMin}+ Pax) — ₹{applyMarkup(p.perPersonPrice, pkg.id).toLocaleString("en-IN")} / person
                    </option>
                  ))}
                </select>
              </div>
