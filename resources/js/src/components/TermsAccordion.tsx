...
export default function TermsAccordion({ termsAccordion, tripName }: TermsAccordionProps) {
  ...
  return (
    <section id="terms" className="relative bg-[#FAF9F6] py-24 px-6 overflow-hidden text-neutral-900 border-t border-neutral-200">
      ...
      <div className="relative z-10 max-w-4xl mx-auto space-y-12">
        <div className="text-center space-y-4">
          ...
          <h2 className="text-4xl md:text-5xl font-black text-neutral-900 tracking-tight font-display uppercase">Before You Travel</h2>
          ...
          {termsAccordion.map((term, index) => {
            ...
            return (
              <div key={term.title} className={`rounded-2xl border transition-all duration-300 shadow-sm ${isOpen ? "bg-white border-brand-sand/40" : "bg-white/80 border-neutral-200 hover:border-neutral-300"}`}>
                ...
                {isOpen && <div className="px-6 pb-6 text-xs sm:text-sm text-neutral-600 leading-relaxed font-light border-t border-neutral-200 pt-4 animate-[fadeIn_0.3s_ease-out]">{term.content}</div>}
              </div>
            );
          })}
        </div>
