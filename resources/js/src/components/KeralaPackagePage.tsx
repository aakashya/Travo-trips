...
export default function KeralaPackagePage({ packageId, onNavigate, onOpenBooking }: KeralaPackagePageProps) {
  ...
  const perPersonRate = applyMarkup(paxPricing.pricePerPerson, pkg.id);
...
Please check availability and send booking voucher details.`;
  ...
  const handleBookNow = () => {
    onOpenBooking(pkg.id, { paxCount: selectedPaxCount, starRating: parseInt(selectedTierId, 10) || undefined });
  };
                ...
                >
                  {activeTier.pricingByPax.map((p) => (
                    <option key={p.paxCount} value={p.paxCount}>
                      {p.paxSlab} — ₹{applyMarkup(p.pricePerPerson, pkg.id).toLocaleString("en-IN")} / person
                    </option>
                  ))}
                </select>
              </div>
