...
export default function SikkimPackagePage({ packageId, onNavigate, onOpenBooking }: SikkimPackagePageProps) {
  ...
  const perPersonRate = isSeasonRate ? paxPricing.seasonPerPerson : paxPricing.offSeasonPerPerson;
  const grandTotal = isSeasonRate ? paxPricing.seasonTotal : paxPricing.offSeasonTotal;
...
✨ Inclusions: MAP Meal Plan (Breakfast + Dinner) & Tsomgo Lake Permits
      ...
      <section className="relative bg-neutral-900 text-white min-h-[380px] flex flex-col justify-end p-6 sm:p-12 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto w-full space-y-4">
          ...
          <div className="pt-2 flex flex-wrap items-center gap-6 text-xs text-amber-200 font-medium border-t border-white/10">
            ...
            <span className="flex items-center gap-1.5"><Hotel className="w-4 h-4 text-sky-400" /> MAP Meal Plan (Daily Breakfast & Dinner)</span>
                ...
                >
                  {activeTier.seasonPricing.map((p) => (
                    <option key={p.paxCount} value={p.paxCount}>
                      {p.paxSlab} — ₹{(isSeasonRate ? p.seasonPerPerson : p.offSeasonPerPerson).toLocaleString("en-IN")} / person
                    </option>
                  ))}
                </select>
              </div>
