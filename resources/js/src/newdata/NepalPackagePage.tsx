...
export default function NepalPackagePage({ packageId, onNavigate, onOpenBooking }: NepalPackagePageProps) {
  ...
  const perPersonBaseRate = useMemo(() => {
    ...
    return slabData ? slabData.pricePerPerson : activeTier.pricingByPax[0].pricePerPerson;
  }, [activeTier, selectedPaxSlab]);
      ...
      <section className="relative bg-neutral-900 text-white min-h-[380px] flex flex-col justify-end p-6 sm:p-12 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto w-full space-y-4">
          ...
          <div className="pt-2 flex flex-wrap items-center gap-6 text-xs text-amber-200 font-medium border-t border-white/10">
            ...
            <span className="flex items-center gap-1.5"><Hotel className="w-4 h-4 text-sky-400" /> CP Meal Plan (Daily Breakfast Included)</span>
          ...
          <div className="lg:col-span-7 space-y-10">
            ...
            <div className="bg-white border border-neutral-200 rounded-3xl p-6 sm:p-8 shadow-sm space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {NEPAL_PAYMENT_POLICY.map((policy, idx) => (
                  <div key={idx} className="p-3.5 bg-[#FAF9F6] border border-neutral-200 rounded-2xl space-y-1">
                    <span className="text-[10px] font-black uppercase text-[#9C753B]">{policy.installment}</span>
                    <p className="text-base font-black text-neutral-900">{policy.percentage}</p>
                    <p className="text-[10px] text-neutral-500">{policy.timing}</p>
                  </div>
                ))}
              </div>

              <div className="pt-2 text-xs text-neutral-600 space-y-1 font-light">
                {NEPAL_GENERAL_NOTES.slice(0, 3).map((note, idx) => (
          ...
          <div className="lg:col-span-5 space-y-6 lg:sticky lg:top-24">
            ...
            <div className="bg-white border-2 border-[#9C753B] rounded-3xl p-6 sm:p-7 shadow-2xl space-y-6 relative overflow-hidden">
              ...
              <div className="space-y-2 pt-2 border-t border-neutral-100">
                ...
                >
                  <option value={1}>1 Pax (Solo Traveler — 2-3 PAX Base Slab: ₹{activeTier.pricingByPax[0]?.pricePerPerson.toLocaleString("en-IN")})</option>
                  <option value={2}>2 Pax (2 Adults Double Sharing — ₹{activeTier.pricingByPax[0]?.pricePerPerson.toLocaleString("en-IN")} / pax)</option>
                  <option value={3}>3 Pax (3 Adults Triple Sharing — ₹{activeTier.pricingByPax[0]?.pricePerPerson.toLocaleString("en-IN")} / pax)</option>
                  <option value={4}>4 Pax (4 Adults / Scorpio AC — ₹{activeTier.pricingByPax[1]?.pricePerPerson.toLocaleString("en-IN")} / pax)</option>
                  <option value={5}>5 Pax (5 Adults / Scorpio AC — ₹{activeTier.pricingByPax[1]?.pricePerPerson.toLocaleString("en-IN")} / pax)</option>
                  <option value={6}>6 Pax (6 Adults Group / Hiace AC Van — ₹{activeTier.pricingByPax[2]?.pricePerPerson.toLocaleString("en-IN")} / pax)</option>
                  <option value={8}>8 Pax (8 Adults Group / Hiace AC Van — ₹{activeTier.pricingByPax[2]?.pricePerPerson.toLocaleString("en-IN")} / pax)</option>
                  <option value={10}>10 Pax (10 Adults Group / Hiace AC Van — ₹{activeTier.pricingByPax[2]?.pricePerPerson.toLocaleString("en-IN")} / pax)</option>
                  <option value={12}>12 Pax (12 Adults Group / Hiace AC Van — ₹{activeTier.pricingByPax[2]?.pricePerPerson.toLocaleString("en-IN")} / pax)</option>
                  <option value={15}>15 Pax (15 Adults Group / Coaster AC — ₹{activeTier.pricingByPax[3]?.pricePerPerson.toLocaleString("en-IN")} / pax)</option>
                  <option value={20}>20 Pax (20 Adults Group / Coaster AC — ₹{activeTier.pricingByPax[3]?.pricePerPerson.toLocaleString("en-IN")} / pax)</option>
                  <option value={25}>25 Pax (25 Adults Large Group / Sutlej Coach — ₹{activeTier.pricingByPax[4]?.pricePerPerson.toLocaleString("en-IN")} / pax)</option>
                  <option value={30}>30 Pax (30 Adults Large Group / Sutlej Coach — ₹{activeTier.pricingByPax[4]?.pricePerPerson.toLocaleString("en-IN")} / pax)</option>
                </select>
              </div>
