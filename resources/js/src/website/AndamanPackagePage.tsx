...
import { ANDAMAN_PACKAGES, AndamanPackage, AndamanHotelCategory, applyAndamanPrice, buildPricingMap } from "./andamanPackages";
...
export default function AndamanPackagePage({ packageId, onNavigate, onOpenBooking }: AndamanPackagePageProps) {
  ...
  const honeymoonFee = (selectedPax === 2 && isHoneymoonSpecial) ? applyAndamanPrice(4500) : 0;
  const totalPrice = pricingObj ? pricingObj.totalPrice + honeymoonFee : 0;
      ...
      <section className="relative bg-neutral-900 text-white min-h-[380px] flex flex-col justify-end p-6 sm:p-12 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto w-full space-y-4">
          ...
          <div className="pt-2 flex flex-wrap items-center gap-6 text-xs text-amber-200 font-medium border-t border-white/10">
            ...
            <span className="flex items-center gap-1.5"><Car className="w-4 h-4 text-sky-400" /> Private AC Cabs</span>
            ...
            <div className="bg-white border-2 border-[#9C753B] rounded-3xl p-6 sm:p-7 shadow-2xl space-y-6 relative overflow-hidden">
              ...
              <div className="space-y-2 pt-2 border-t border-neutral-100">
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  >
                    ...
                    <span className="text-[9px] opacity-80 block">CP Plan • Romantic Couple Extras</span>
                  </button>
                </div>
              </div>
              ...
              <div className="space-y-2 pt-2 border-t border-neutral-100">
                <label className="text-xs font-bold uppercase tracking-wider text-neutral-700 flex items-center justify-between">
                  ...
                  <span className="text-[#9C753B] font-black">{selectedPlanType === "CP" ? "CP (Breakfast Only)" : "MAP (Breakfast + Dinner)"}</span>
                </label>
                <div className="grid grid-cols-2 gap-2">
                  <button
                    type="button"
                  ...
                  >
                    <span className="text-[10px] font-bold block">CP Plan</span>
                    <span className="text-[9px] opacity-80 block">Daily Hotel Breakfast</span>
                  </button>
                  ...
                  >
                    <span className="text-[10px] font-bold block">MAP Plan</span>
                    <span className="text-[9px] opacity-80 block">{isHoneymoonSpecial ? "N/A for Honeymoon" : "Breakfast + Dinner Included"}</span>
                  </button>
                </div>
              </div>

              {/* STEP 1: Select Adult Travelers */}
              <div className="space-y-2 pt-2 border-t border-neutral-100">
                <label htmlFor="andaman-travelers" className="text-xs font-bold uppercase tracking-wider text-neutral-700 flex items-center justify-between">
                  <span className="flex items-center gap-1.5"><Users className="w-4 h-4 text-[#9C753B]" /> 1. Adult Travelers:</span>
                  <span className="text-[#9C753B] font-black">{selectedPax} {selectedPax === 1 ? 'Adult' : 'Adults'}</span>
                </label>

                <div className="relative">
                  <select
                    id="andaman-travelers"
                    value={selectedPax}
                    disabled={isHoneymoonSpecial}
                    onChange={(event) => {
                    ...
                    className="w-full appearance-none bg-[#FAF9F6] border border-neutral-200 rounded-xl px-4 py-3.5 pr-11 text-sm font-black text-neutral-900 focus:outline-none focus:border-[#9C753B] focus:ring-2 focus:ring-[#9C753B]/10 transition-colors shadow-sm disabled:bg-neutral-100 disabled:text-neutral-500 disabled:cursor-not-allowed"
                  >
                    {PAX_OPTIONS.map((pax) => (
                      <option key={pax} value={pax}>
                        {pax} {pax === 1 ? "Adult Traveler" : "Adult Travelers"}
                      </option>
                    ))}
                  </select>
                  <ChevronDown className="w-4 h-4 text-[#9C753B] absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none" />
                </div>
                {isHoneymoonSpecial && (
                  <p className="text-[9px] font-bold text-rose-700">Honeymoon packages are fixed for two travelers. Select Standard Package to change the group size.</p>
                )}
              </div>
                ...
                <div className="p-3.5 rounded-2xl bg-amber-50/80 border border-amber-300 space-y-2">
                  <label className="flex items-center justify-between cursor-pointer">
                    <span className="text-xs font-bold text-amber-900 flex items-center gap-1.5">
                      <Sparkles className="w-4 h-4 text-amber-600" /> Include Honeymoon Inclusions Package (+₹6,750)
                    </span>
                    <input 
                      type="checkbox" 
